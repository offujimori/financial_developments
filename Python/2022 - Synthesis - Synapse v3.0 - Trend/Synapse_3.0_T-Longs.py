...
def ORDERS_trend_long(psar_df, signals_df):
    ...
    if timestamp != psar_df['timestamp'].iloc[-1]:
        ...
        if not pd.isna(psar_df['psar up'].iloc[-1]):
            if psar_df['psar up'].iloc[-2] > signals_df['psar'].iloc[-1]:                                                        #SE PSAR ANTERIOR AO DE ANDAMENTO FOR MAIOR QUE O GATILHO ANTERIOR
                ...
                entry = float(EXCHANGE.price_to_precision(symbol=ticker, price=psar_df['psar up'].iloc[-1])) - ticker_increment  ##TRANSFORMAR EM FLOAT PORQUE O DADO RECEBIDO É EM STRING
                entry_format = EXCHANGE.price_to_precision(symbol=ticker, price=entry)
                order_data = EXCHANGE.create_limit_buy_order(symbol=ticker, amount=lot, price=entry_format)
                #order = int(order_data['info']['id'])
                print('\nBUY ORDER CREATED: ', order_data, '\nID: ', int(order_data['info']['id']))

                sl = signals_df['lowest'].iloc[-1]
            ...
            order = None                                                                                                #SÓ JOGADA PARA CONTORNAR BUG DO SERVIDOR
            order_sl = None
