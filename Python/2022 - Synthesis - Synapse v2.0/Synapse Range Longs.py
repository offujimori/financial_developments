...
timestamp = None
def ORDERS_range_long(df_major, df_minor, delta):
    global timestamp
    print("\nChecking for **RANGE LONG** signals")

    if timestamp != df_major['timestamp'].iloc[-1]:
        timestamp = df_major['timestamp'].iloc[-1]

        if not pd.isna(df_major['psar down'].iloc[-1]):
            if df_minor['signal down'].iloc[-1] != 0:
                ...
                order_tp = EXCHANGE.create_limit_sell_order(symbol=TICKER, amount=LOT, price=price_tp_format)
                print('\nMAJOR DOWN // MINOR DOWN // Range BUY...: ', order_market)
                print('MAJOR DOWN // MINOR DOWN // Range BUY TP...: ', order_tp)
...
def BOT_run():
    ...
    ORDERS_range_long(df_major=PSAR_data_major, df_minor=PSAR_data_minor, delta=DELTA_closed)
