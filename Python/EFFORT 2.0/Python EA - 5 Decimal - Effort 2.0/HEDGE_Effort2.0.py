...
try:
    ...
    def GridVerify():
        if Recent_High() >= Grid_Max_Price or Recent_Low() <= Grid_Minimum_Price:
            ...
            for Y in Buy_Cycle_ORDERS + Sell_Cycle_ORDERS:
                try:
                    Order_Cancel(Y)
                except:
    ...
    def Current_Keytrade_ClientID():
        a = Strategy_KeyTrade_Name + str(Strategy_Keytrade_Index)
        return a
    ###########
    ...
    def CycleLots(X):
        if First_Trigger == -1:
            Lots = {"C1": -100, "C2": 185, "C3": -242, "C4": 448, "C5": -829, "C6": 1534, "C7": -2838, "C8": 5250}#, "C9": -9712, "C10": 1797, "C11": -3324, "C12": 6149}#, "C13": -302284, "C14": 589454, "C15": -1149435}
            CycleLot = Lots.get(str(X))
            return CycleLot
        if First_Trigger == 1:
            Lots = {"C1": 100, "C2": -185, "C3": 242, "C4": -448, "C5": 829, "C6": -1534, "C7": 2838, "C8": -5250}#, "C9": 971, "C10": -1797, "C11": 3324, "C12": -6149}#, "C13": 302284, "C14": -589454, "C15": 1149435}
            CycleLot = Lots.get(str(X))
            return CycleLot
    ########################################
    ...
    HEDGE_Grid_TOP = Grid_Max_Price
    HEDGE_Grid_Channel_High = round(float(input("\nDEFINE CHANNEL HIGH -> ")), Price_Decimal) #round(Grid_Minimum_Price + (2 * HEDGE_Grid_Size_Box()), Price_Decimal)
    HEDGE_Grid_Channel_Low = round(float(input("DEFINE CHANNEL LOW -> ")), Price_Decimal) #round(Grid_Minimum_Price + (1 * HEDGE_Grid_Size_Box()), Price_Decimal)
    HEDGE_Grid_BOTTOM = Grid_Minimum_Price
    ##############################################
    ################ OUTPUT STOP GRID #################
    print('')
    print('----- STOP GRID PRICES -----')
    print('TOP: ', HEDGE_Grid_TOP)
    print('CHANNEL HIGH: ', HEDGE_Grid_Channel_High)
    print('*')
    print('CHANNEL LOW: ', HEDGE_Grid_Channel_Low)
    print('BOTTOM: ', HEDGE_Grid_BOTTOM)
    print('----------------------------')
    ##############################################
    ################ GRID VARIABLES #################
    Buy_Price = HEDGE_Grid_Channel_Low
    Buy_Target = Grid_Max_Price
    Buy_Loss = Grid_Minimum_Price
    #
    Sell_Price = HEDGE_Grid_Channel_High
        ...
        try:
            while Recent_High() < Grid_Max_Price and Recent_Low() > Grid_Minimum_Price:
                ...
                if Cycle_Switch == 0 and Trade_Last_Open() != 0 and C1 == False:
                    ...
                    Buy_Cycle_ORDERS.append(Current_Keytrade_ClientID()) #1
                    Trades_ClientIDs.append(Current_Trade_ClientID())

                    ##SET SELL##
                    MITOrder(Sell_Price, Initial_Lot_Sell)
                    Sell_Cycle_ORDERS.append(Current_Keytrade_ClientID()) #2
                    Trades_ClientIDs.append(Current_Trade_ClientID())
                ...
                if Cycle_Switch == 0 and C1 == True:
                    ...
                    if Order_Details(Buy_Cycle_ORDERS[0])["state"] != "PENDING":
                        Cycle_Switch = 1
                        First_Trigger = 1
                        Order_Cancel(Sell_Cycle_ORDERS[0])

                    elif Order_Details(Sell_Cycle_ORDERS[0])["state"] != "PENDING":
                        Cycle_Switch = -1
                        First_Trigger = -1
                        Order_Cancel(Buy_Cycle_ORDERS[0])
                ...
                if Cycle_Switch == -1:
                    ...
                    Buy_Cycle_ORDERS.append(Current_Keytrade_ClientID())
                    Trades_ClientIDs.append(Current_Trade_ClientID())
                    ...
                    Sell_Cycle_ORDERS.append(Current_Keytrade_ClientID())
                    Trades_ClientIDs.append(Current_Trade_ClientID())
                    Triggers.remove(Triggers[0])
                    Cycle_Loop_Switch = True
                    Cycle_Switch = -1
