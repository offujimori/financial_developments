...
try:
    ...
    #DistanceStop = input("STOP DISTANCE [Price Units 0.xxxxx]: ")
    TakeProfit = float(input("TP DISTANCE [Price Units 0.xxxxx]: "))
    print("-----------------------", "\n")
    ...
    CurrentCandle_DAY = None
    TP = None

    def Instrument_CANDLES_Current():
        params = {"price": "B", "count": 2, "granularity": Granularity, "weeklyAlignment": "Sunday",
                  "dailyAlignment": "17"}
    ...
    def Instrument_CANDLES_Pre_Previous():
        params = {"price": "B", "count": 3, "granularity": Granularity, "weeklyAlignment": "Sunday",
                  "dailyAlignment": "17"}
        r = instruments.InstrumentsCandles(instrument=Instrument, params=params)
        rv = Client.request(r)
        result = rv["candles"]
    ...
    def Order_MKT(Units):  # , OrderClientExtension, TradeClientExtension):
        global TP
        if Units > 0:
            TP = round(CurrentCandle_DATA["CLOSE"] + TakeProfit, Price_Decimal)
        elif Units < 0:
            TP = round(CurrentCandle_DATA["CLOSE"] - TakeProfit, Price_Decimal)
        data = {
            "order": {
                ...
                #"stopLossOnFill": {"distance": DistanceStop},
        ...
        print("Order Market Opened. TP: ", str(TP))
        TP = None
    ...
    while True:
        try:
            ...
            print(CurrentCandle_DATA["DAY"], CurrentCandle_DAY)

            if CurrentCandle_DATA["DAY"] != CurrentCandle_DAY:

                Order_MKT(Units_Size)
                Order_MKT(-Units_Size)
                CurrentCandle_DAY = CurrentCandle_DATA["DAY"]
