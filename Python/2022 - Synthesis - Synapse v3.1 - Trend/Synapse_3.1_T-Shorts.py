...
def ORDERS_trend_short(psar_df, signals_df):
    ...
    if timestamp != psar_df['timestamp'].iloc[-1]:
        ...
        if not pd.isna(psar_df['psar up'].iloc[-1]) and not pd.isna(psar_df['psar down'].iloc[-2]):
            if signals_df['type'].iloc[-1] == -1:

                delta = abs(float(signals_df['psar down'].iloc[-1]) - float(signals_df['highest'].iloc[-2])) ##PEGAR O CRUZAMENTO QUE TEVE MAIS O EXTREME POINT ANTERIOR
                count += 1
                order_sl += delta
                print('******** SHORT COUNT: ', count)

                if count == 5:
                    order_data = EXCHANGE.create_market_sell_order(symbol=ticker, amount=lot)
